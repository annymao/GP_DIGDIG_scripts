using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using System.Linq;
public class SetSprite : Photon.MonoBehaviour {
	public Sprite[] playerSprite;
	// Use this for initialization
	// The name of the sprite sheet to use
	public string SpriteSheetName;

	// The name of the currently loaded sprite sheet
	private string LoadedSpriteSheetName;

	// The dictionary containing all the sliced up sprites in the sprite sheet
	private Dictionary<string, Sprite> spriteSheet;

	// The Unity sprite renderer so that we don't have to get it multiple times
	private SpriteRenderer spriteRenderer;

	public Sprite mySprite;

	void Start () {
		Debug.Log("...................");
		gameObject.GetComponent<SpriteRenderer> ().sprite = mySprite;
		Debug.Log (this.spriteRenderer.sprite.name);
	}
	
	// Update is called once per frame
	void Update () {
		
	}
	public void MySprite(int id){
		gameObject.GetComponent<PhotonView> ().RPC ("SetPlayerSprite", PhotonTargets.All, id);
	}
	[PunRPC]
	public void SetPlayerSprite(int player){
		gameObject.GetComponent<SpriteRenderer> ().sprite = playerSprite[player];
		mySprite = playerSprite [player];
		this.spriteRenderer = GetComponent<SpriteRenderer>();
		if (player == 0)
			SpriteSheetName = "fairy_f01";
		else if (player == 1)
			SpriteSheetName = "fairy_f02";
		else if (player == 2)
			SpriteSheetName = "fairy_f03";
		else if (player == 3)
			SpriteSheetName = "fairy_f04";
		else if (player == 4)
			SpriteSheetName = "fairy_m01";
		else if (player == 5)
			SpriteSheetName = "fairy_m02";
		else if (player == 6)
			SpriteSheetName = "fairy_m03";
		else if (player == 7)
			SpriteSheetName = "fairy_m04";
		Debug.Log (this.spriteRenderer.sprite.name);
		this.LoadSpriteSheet();
	}
	private void LateUpdate()
	{
		// Check if the sprite sheet name has changed (possibly manually in the inspector)
		if (this.LoadedSpriteSheetName != this.SpriteSheetName)
		{
			Debug.Log ("////");
			// Load the new sprite sheet
			this.LoadSpriteSheet();
		}

		// Swap out the sprite to be rendered by its name
		// Important: The name of the sprite must be the same!
		Debug.Log (this.spriteRenderer.sprite.name);
		this.spriteRenderer.sprite = this.spriteSheet[this.spriteRenderer.sprite.name];
	}

	// Loads the sprites from a sprite sheet
	private void LoadSpriteSheet()
	{
		Debug.Log ("Load Sprite Sheet");
		Debug.Log (this.SpriteSheetName);
		// Load the sprites from a sprite sheet file (png). 
		// Note: The file specified must exist in a folder named Resources
		var sprites = Resources.LoadAll<Sprite>(this.SpriteSheetName);
		this.spriteSheet = sprites.ToDictionary(x => x.name, x => x);

		// Remember the name of the sprite sheet in case it is changed later
		this.LoadedSpriteSheetName = this.SpriteSheetName;
		//gameObject.GetComponent<SpriteRenderer> ().sprite = mySprite;
	}
}
